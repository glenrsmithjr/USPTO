...
def process_citations(cleanDataDir, filePath_uspatentcitation, filePath_foreigncitation):
	...
	'''
		...
		2. The list of columns to keep when reading in the file
	...
	'''
	toChange = [
		[filePath_uspatentcitation, ['patent_id', 'citation_id', 'category', 'sequence', 'name', 'kind'],
		 	{'patent_id': 'outgoing_citation_id', 'citation_id': 'incoming_citation_id'}, 'usCitations.tsv', 'us'],
		[filePath_foreigncitation, ['patent_id', 'number', 'category', 'sequence', 'country'],
		 	{'patent_id': 'outgoing_citation_id', 'number': 'incoming_citation_id'}, 'foreignCitations.tsv', 'foreign']
	]
...
def createCitationTable(cleanDataDir, citations, tableName, usForeignMarker):
	...
	if usForeignMarker == 'us':
		citations = citations.rename(columns = {'name': 'incoming_citation_name', 'kind': 'incoming_citation_kind'})
	...
	citations['category'] = citations['category'].replace({"": '0'})
	citations['category_id'] = pd.np.where(citations.category.str.contains('0'), 0,
	...
	return citations[['outgoing_citation_id', 'incoming_citation_id']]

def getNumCitationsUS(cleanDataDir, us_citations):
	# Calculate # of outgoing citations to US patents
	us_us_outgoing_cits = us_citations.groupby('outgoing_citation_id').size().reset_index(name='outgoingCitationsUS')
	...
	us_us_incoming_cits = us_citations.groupby('incoming_citation_id').size().reset_index(name='incomingCitationsUS')
	...
	patents = patents.merge(us_us_outgoing_cits, left_on = 'patent_id', right_on = 'outgoing_citation_id', how = 'left')
	patents = patents.merge(us_us_incoming_cits, left_on = 'patent_id', right_on = 'incoming_citation_id', how = 'left')
	# Remove irrelevant columns
	patents = patents.drop(columns = ['outgoing_citation_id', 'incoming_citation_id'])
	# Save patents table
...
def getNumCitationsForeign(cleanDataDir, foreign_citations):
	...
	us_foreign_outgoing_cits = foreign_citations.groupby('outgoing_citation_id').size().reset_index(name='outgoingCitationsForeign')

	patents = pd.read_csv(cleanDataDir + 'patent.tsv', sep = '\t')
	# Join all data into patents table
	patents = patents.merge(us_foreign_outgoing_cits, left_on = 'patent_id', right_on = 'outgoing_citation_id', how = 'left')
	# Remove irrelevant columns
	patents = patents.drop(columns = ['outgoing_citation_id'])
	# Save patents table
	patents.to_csv(cleanDataDir + 'patent.tsv', sep = '\t', index=False)
...
def patentCitationCategory(cleanDataDir):
	...
	df = pd.DataFrame(list(categories.items()), columns = ['citation_category_id', 'citation_category_description'])
